...
class DataTests(unittest.TestCase):
    def test_extract_image_id_from_mask_name(self):
        self.assertEqual(extract_image_id(Path("ISIC_0012345_segmentation.png")), "ISIC_0012345")

    def test_split_is_deterministic_and_disjoint(self):
        samples = [Sample(Path(f"{i}.jpg"), Path(f"{i}.png"), f"ISIC_{i:07d}") for i in range(20)]
        ...
        ids = [{item.image_id for item in group} for group in first]
    ...
    def test_invalid_split_is_rejected(self):
        samples = [Sample(Path(f"{i}.jpg"), Path(f"{i}.png"), str(i)) for i in range(10)]
    ...
    def test_offline_augmentations_are_identified(self):
        ...
        self.assertTrue(is_augmented_path(Path("ISIC_0000001_segmentation_saturated.png")))
